...
const ui = {
    ...
    */
    ...
    displayFactionFields: function(factionKey) {
        ...
        // Replace static text with editable input for faction title
        const titleElement = document.getElementById('faction-name-display');
        if (titleElement) {
            // Create an input element
            const titleInput = document.createElement('input');
            titleInput.type = 'text';
            titleInput.className = 'input faction-title-input';
            titleInput.id = `faction-${factionKey}-title-input`;
            titleInput.value = factionData.name;
            titleInput.style.fontWeight = 'bold';
            titleInput.style.fontSize = '1.17em'; // Matches h3 size
            titleInput.style.margin = '0';
            titleInput.style.width = '100%';
            
            // Store reference to the original faction data
            titleInput.dataset.factionKey = factionKey;
            
            // Replace the h3 content with the input
        ...
        Object.entries(factionData.fields).forEach(([fieldKey, fieldConfig]) => {
             ...
             // Use a unique ID based on faction and field key
             ...
             input.type = 'text';
             input.className = 'input faction-data-input'; // Add class for easy selection
             input.id = inputId;
             input.placeholder = fieldConfig.placeholder;
             // Store block number and key for writing operations
             input.dataset.sector = factionData.sector; // Store sector
             input.dataset.block = fieldConfig.block; // Use 'block' from FIELD_MAP
             input.dataset.key = fieldConfig.key; // Use 'key' from FIELD_MAP
             input.dataset.fieldKey = fieldKey; // Store the original field key
    ...
    displayAllegianceFields: function(allegianceKey) {
        ...
        // Replace static text with editable input for allegiance title
        const titleElement = document.getElementById('allegiance-name-display');
        if (titleElement) {
            // Create an input element
            const titleInput = document.createElement('input');
            titleInput.type = 'text';
            titleInput.className = 'input allegiance-title-input';
            titleInput.id = `allegiance-${allegianceKey}-title-input`;
            titleInput.value = allegianceData.name;
            titleInput.style.fontWeight = 'bold';
            titleInput.style.fontSize = '1.17em'; // Matches h3 size
            titleInput.style.margin = '0';
            titleInput.style.width = '100%';
            
            // Store reference to the original allegiance data
            titleInput.dataset.allegianceKey = allegianceKey;
        ...
        Object.entries(allegianceData.fields).forEach(([fieldKey, fieldConfig]) => {
             ...
             const input = document.createElement('input');
             input.type = 'text';
             input.className = 'input allegiance-data-input';
             input.id = inputId;
             input.placeholder = fieldConfig.placeholder;
             // Store block number and key for writing operations
             input.dataset.sector = allegianceData.sector; // Store sector
             input.dataset.block = fieldConfig.block; // Use 'block' from FIELD_MAP
             input.dataset.key = fieldConfig.key;     // Use 'key' from FIELD_MAP
